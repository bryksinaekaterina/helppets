...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (happyStoriesGrid) {
        fetch('https://help-pets.uz/wp-json/wp/v2/happy_story?_embed&acf_format=standard&per_page=2')
            ...
            .catch(error => {
                console.error('Ошибка при получении счастливых историй (главная):', error);
            });
    } else {
        console.error('Контейнер .happy-stories-grid не найден (главная).');
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    const allHappyStoriesGrid = document.querySelector('.all-happy-stories-grid');

    if (allHappyStoriesGrid) {
        fetch('https://help-pets.uz/wp-json/wp/v2/happy_story?_embed&acf_format=standard')
            .then(response => response.json())
            .then(stories => {
                console.log('Данные счастливых историй (все):', stories);
                stories.forEach(story => {
                    const acf = story.acf;
                    const title = acf?.story_title;
                    const excerpt = acf?.brief_description;
                    ...
                    const storyCard = document.createElement('div');
                    storyCard.classList.add('all-happy-story-card');
                    ...
                    allHappyStoriesGrid.appendChild(storyCard);
                });
            })
            .catch(error => {
                console.error('Ошибка при получении счастливых историй (все):', error);
            });
    } else {
        console.error('Контейнер .all-happy-stories-grid не найден (все).');
    }
});